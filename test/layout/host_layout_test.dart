...
void main() {
  group('HostLayout.computePortCenters', () {
    test('returns empty map for zero ports', () {
      final positions = HostLayout.computePortCenters(0, const Size(800, 400));
      expect(positions, isEmpty);
    });

    test('returns correct number of ports for 5 ports', () {
      final positions = HostLayout.computePortCenters(5, const Size(800, 400));
      expect(positions.length, 5);
      expect(positions.keys, containsAll([1, 2, 3, 4, 5]));
    });

    test('returns correct number of ports for 12 ports', () {
      final positions = HostLayout.computePortCenters(12, const Size(800, 400));
      expect(positions.length, 12);
    });

    test('single port is centered above host (90 degrees)', () {
      final positions = HostLayout.computePortCenters(1, const Size(800, 400));
    ...
    test(
      'first port is to the right, last port is to the left (30 to 150 deg)',
      () {
    ...
    test('centerYFactor shifts host center vertically', () {
      final size = const Size(800, 400);
      final positions06 = HostLayout.computePortCenters(
        3,
        size,
        centerYFactor: 0.6,
      );
      final positions04 = HostLayout.computePortCenters(
        3,
        size,
        centerYFactor: 0.4,
      );
    ...
    test('two ports are symmetric around center', () {
      final positions = HostLayout.computePortCenters(2, const Size(800, 400));
      final centerX = 400.0;
  ...
  group('HostLayout.computeCenterSize', () {
    test('returns 30% of minimum dimension', () {
      expect(
        HostLayout.computeCenterSize(const Size(800, 400)),
        closeTo(120, 0.1),
      );
      expect(
        HostLayout.computeCenterSize(const Size(400, 800)),
        closeTo(120, 0.1),
      );
    });
  });
